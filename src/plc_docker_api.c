...
static plcCurlBuffer *plcCurlRESTAPICall(plcCurlCallType cType,
	...
	if (curl) {
		...
		if (res != CURLE_OK) {
			...
			backendlog(LOG, "Curl Request with type: %d, url: %s", cType, fullurl);
			backendlog(LOG, "Curl Request with http body: %s\n", body);
			backendlog(LOG, "Curl Request costs "
				UINT64_FORMAT
				"ms", elapsed_us / 1000);
			...
			backendlog(DEBUG1, "CURL response code is %ld. CURL response message is %s", http_code, buffer->data);
		}
...
int plc_docker_create_container(runtimeConfEntry *conf, char **name, int container_id, char **uds_dir) {
	...
	if (res == 201) {
		...
		backendlog(LOG, "Docker fails to create container, response: %s", response->data);
		snprintf(backend_error_message, sizeof(backend_error_message),
	...
	if (res < 0) {
		backendlog(DEBUG1, "Error parsing container ID during creating container with errno %d.", res);
		snprintf(backend_error_message, sizeof(backend_error_message),
...
int plc_docker_start_container(const char *name) {
	...
	if (res == 204 || res == 304) {
		...
		backendlog(DEBUG1, "start docker container %s failed with errno %d.", name, res);
		snprintf(backend_error_message, sizeof(backend_error_message),
...
int plc_docker_kill_container(const char *name) {
	...
	backendlog(FATAL, "Not implemented yet. Do not call it.");
...
int plc_docker_inspect_container(const char *name, char **element, plcInspectionMode type) {
	...
	if (res != 200) {
		backendlog(LOG, "Docker cannot inspect container, response: %s", response->data);
		snprintf(backend_error_message, sizeof(backend_error_message),
...
static int docker_inspect_string(char *buf, char **element, plcInspectionMode type) {
	int i;
	backendlog(DEBUG1, "plcontainer: docker_inspect_string:%s", buf);
	...
	if (type == PLC_INSPECT_NAME) {
		...
		*element = pstrdup(namestr);
		...
		for (i = 0; i < arraylen; i++) {
			...
			if (PortBindingObj == NULL) {
				backendlog(WARNING, "failed to get json \"PortBinding\" field.");
				return -1;
			}
			struct json_object *HostPortObj = NULL;
			if (!json_object_object_get_ex(PortBindingObj, "HostPort", &HostPortObj)) {
				backendlog(WARNING, "failed to get json \"HostPort\" field.");
				return -1;
			}
		...
		if (!json_object_object_get_ex(response, "State", &StateObj)) {
			backendlog(WARNING, "failed to get json \"State\" field.");
			return -1;
		}
#ifdef DOCKER_API_LOW
		struct json_object *RunningObj = NULL;
		if (!json_object_object_get_ex(StateObj, "Running", &RunningObj)) {
			backendlog(WARNING, "failed to get json \"Running\" field.");
			return -1;
		}
		...
		*element = pstrdup(RunningStr);
		...
		if (!json_object_object_get_ex(StateObj, "Status", &StatusObj)) {
			backendlog(WARNING, "failed to get json \"Status\" field.");
			return -1;
		}
		...
		*element = pstrdup(StatusStr);
		...
		if (!json_object_object_get_ex(StateObj, "OOMKilled", &OOMKillObj)) {
			backendlog(WARNING, "failed to get json \"OOMKilled\" field.");
			return -1;
		}
		...
		*element = pstrdup(OOMKillStr);
		...
		backendlog(LOG, "Error PLC inspection mode, unacceptable inpsection type %d", type);
		return -1;
	}
